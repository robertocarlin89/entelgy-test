package com.entelgy.controller;

import static org.assertj.core.api.Assertions.assertThat;
import static org.junit.jupiter.api.Assertions.*;

import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.InjectMocks;
import org.mockito.Mock;
import org.mockito.Mockito;
import org.mockito.junit.jupiter.MockitoExtension;
import org.springframework.http.ResponseEntity;

import com.entelgy.clients.impl.ClientReqResImpl;
import com.entelgy.config.ApplicationProperties;
import com.entelgy.dto.DataResponseDTO;
import com.entelgy.dto.EmployeeDTO;
import com.entelgy.service.ConsumerService;
import static org.mockito.Mockito.when;

import java.util.ArrayList;
import java.util.List;

@ExtendWith(MockitoExtension.class)
class ConsumerControllerTest {
	
	@InjectMocks
	private ConsumerController consumerController;
	
	@Mock
	private ConsumerService consumerService;
	
	@BeforeEach
	void setUp() throws Exception {
		Mockito.mock(ConsumerController.class);
		Mockito.mock(ConsumerService.class);
	}

	@Test
	void testGetInformationApi() {
		DataResponseDTO response = new DataResponseDTO();
		List<EmployeeDTO> data = new ArrayList<>();
		EmployeeDTO obj1 = new EmployeeDTO();
		obj1.setId(1);
		obj1.setEmail("roberto@gmail.com");
		obj1.setLastName("Carlin");
		data.add(obj1 );
		response.setData(data );
		when(consumerService.getInformationApi()).thenReturn(response);
		DataResponseDTO responseAPI = consumerController.getInformationApi();
		assertThat(responseAPI.getData().size()).isEqualTo(1);
	}

}
